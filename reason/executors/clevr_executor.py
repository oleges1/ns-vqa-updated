...
def check_close(o1, o2):
    dist = np.sqrt(((np.array(o1['position']) - np.array(o2['position'])) ** 2).sum())
    if o1['size'] == 'small':
        return dist < SMALL_DIST_THR, dist
    return dist < LARGE_DIST_THR, dist
...
class ClevrExecutor:
    ...
    def _register_modules(self):
        ...
        self.modules['relate[close]'] = self.relate_close
        self.modules['relate[far]'] = self.relate_far
    ...
    def relate_close(self, obj, scene):
        if type(obj) == dict and 'position' in obj and type(scene) == list:
            output = []
            distances = []
            for o in scene:
                value, dist = check_close(obj, o)
                if o['id'] != obj['id'] and value:
                    output.append(o)
                    distances.append(dist)
            return [o for _, o in sorted(zip(distances, output))]
        return 'error'
    
    def relate_far(self, obj, scene):
        if type(obj) == dict and 'position' in obj and type(scene) == list:
            output = []
            distances = []
            for o in scene:
